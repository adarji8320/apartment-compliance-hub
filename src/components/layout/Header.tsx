    ...
    <header className="bg-brand sticky top-0 z-50 w-full border-b text-white shadow-md">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <span
              className={cn(
                'block w-6 h-0.5 bg-white mb-1 transition-all',
                open && 'rotate-45 translate-y-1.5',
              )}
            />
            <span
              className={cn('block w-6 h-0.5 bg-white mb-1 transition-all', open && 'opacity-0')}
            />
            <span
              className={cn(
      ...
      {open && (
        <nav
          ...
          className="lg:hidden bg-brand/90 px-4 pb-4 flex flex-col gap-3 text-sm font-medium"
        >
          {navLinks.map((link) => (
            <Link
              key={link.to}
          ...
          {isAuthenticated && user ? (
            ...
            <Link
              to="/login"
              onClick={() => setOpen(false)}
              className={cn(navButtonClass, 'w-fit')}
            >
              Login
            </Link>
          )}
